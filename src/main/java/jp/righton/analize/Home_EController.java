...
public class Home_EController {
    //　　　　　　　　　　　　　全社順位、　　　ブロック順位、　　　　自店順位、　　　部門、　
    record TaskItem_E(int rankCompany, int rankBlock, int rankStore, Cell Group,
    ...
    public String analyze(@RequestPart("UpLoadFile_E") MultipartFile UpLoadFile)throws EncryptedDocumentException, IOException {
        ...
        for (int i = 5; i <= sheet.getLastRowNum(); i++) {
            ...
            if (salesPoint >= 1.0 && rankCompany <= 3.0 && rankStore <= 3.0) {
            ...
            if (rankCompany <= 5.0 && rankBlock <= 5.0 && rankStore >= 7.0) {
                String itemLink = "https://right-on.co.jp/search?q=" + itemNumber;
            ...
            if (rankCompany >= 10.0 && rankBlock >= 10.0 && salesPoint >= 1.0 && rankStore <= 3.0) {
                String itemLink = "https://right-on.co.jp/search?q=" + itemNumber;
